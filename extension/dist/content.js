"use strict";(()=>{var E=null,k="",T="professional",C="short",A=null,x=null,y=null,v=!1;function H(o){let t=o,e=[".expandable-text-box",".feed-shared-inline-show-more-text",'p[componentkey*="feed-commentary"]','[class*="commentary"]',".feed-shared-update-v2__description-text",".update-components-text"];for(;t&&!(t.tagName==="BODY"||t.tagName==="HTML");){for(let r of e){let l=t.querySelector(r);if(l){let g=!1,p=l;for(;p&&p!==t;){let d=(p.className||"").toLowerCase(),m=(p.getAttribute("componentkey")||"").toLowerCase();if(d.includes("comment")&&!d.includes("commentary")||m.includes("comment")&&!m.includes("commentary")||p.closest(".comments-comment-item")||p.closest(".comment-item")){g=!0;break}p=p.parentElement}if(!g){let d=(l.textContent||"").trim();if(d=d.replace(/\bsee\s+more\b/gi,"").trim(),d.length>10)return console.log(`AI Reply Extension: Found post text via climbing fast-path selector "${r}":`,d),d}}}let n=t.getAttribute("role")||"",i=t.getAttribute("componentkey")||"";if(t.tagName==="ARTICLE"||t.hasAttribute("data-urn")||n==="listitem"||i.includes("FeedType_MAIN_FEED")||t.classList.contains("feed-shared-update-v2")||t.classList.contains("occludable-update")||t.classList.contains("feed-shared-update"))break;t=t.parentElement}return""}function I(o){let t=H(o);if(t)return t;console.log("AI Reply Extension: Fast-path failed. Falling back to structured climbers...");let e=o,n=null;for(;e;){let u=e.getAttribute("role")||"",a=e.getAttribute("componentkey")||"",s=(e.className||"").toLowerCase(),c=e.classList.contains("comments-comment-item")||e.classList.contains("comment-item")||e.closest(".comments-comment-item")||e.closest(".comment-item")||(e.getAttribute("data-id")||"").startsWith("comment-"),b=s.includes("comment")&&!s.includes("commentary");if(!c&&!b&&(e.tagName==="ARTICLE"||e.hasAttribute("data-urn")||u==="listitem"||a.includes("FeedType_MAIN_FEED")||e.classList.contains("feed-shared-update-v2")||e.classList.contains("occludable-update")||e.classList.contains("feed-shared-update"))){n=e;break}e=e.parentElement}if(!n){let u=o;for(let a=0;a<16;a++)u.parentElement&&(u=u.parentElement);n=u}if(!n)return console.warn("AI Reply Extension: Could not resolve parent post card."),"";console.log("AI Reply Extension: Found post card container:",n.tagName,n.className);let i=null,r=o;for(;r&&r.parentElement&&r.parentElement!==n;)r=r.parentElement;r&&r.parentElement===n&&(i=r);let l=null,g=n.querySelector('.feed-shared-actor, .update-components-actor, [class*="actor"], [class*="header"]');if(g)l=g;else{let u=n.querySelector('a[href*="/in/"], a[href*="/company/"]');if(u){let a=u.closest('.feed-shared-actor, .update-components-actor, [class*="actor"], [class*="header"]');if(a)l=a;else{let s=u,c=0;for(;s&&s.parentElement&&s.parentElement!==n&&c<4;)s=s.parentElement,c++;s&&s.parentElement===n&&(l=s)}}}let p=[".expandable-text-box",".feed-shared-inline-show-more-text",'p[componentkey*="feed-commentary"]','[class*="commentary"]'];for(let u of p){let a=n.querySelector(u);if(a){let s=i&&i.contains(a),c=u!==".expandable-text-box"&&l&&l.contains(a);if(!s&&!c){let b=(a.textContent||"").trim();if(b=b.replace(/\bsee\s+more\b/gi,"").trim(),b.length>15)return console.log(`AI Reply Extension: Found post text via primary selector "${u}":`,b),b}}}console.log("AI Reply Extension: Primary selectors failed. Falling back to class-agnostic scanner...");let d=n.querySelectorAll("p, span"),m=[];d.forEach(u=>{let a=u;if(i&&i.contains(a)||l&&l.contains(a)||a.closest('a[href*="/in/"], a[href*="/company/"]')||a.closest("#ai-reply-assistant-root")||a.closest('div[contenteditable="true"]')||a.tagName==="TEXTAREA"||a.closest('[class*="social-"]')||a.closest('[class*="action-bar"]')||a.closest('[class*="footer"]')||a.closest(".feed-shared-social-action-bar")||a.closest(".feed-shared-social-counts"))return;let s=(a.textContent||"").trim();s=s.replace(/\bsee\s+more\b/gi,"").trim();let c=s.toLowerCase();if(s.length>10&&!c.includes("like")&&!c.includes("comment")&&!c.includes("repost")&&!c.includes("send")&&!/^\d+$/.test(s)&&!c.includes("reactions")){let b=m.findIndex(L=>L.includes(s)||s.includes(L));b!==-1?s.length>m[b].length&&(m[b]=s):m.push(s)}});let f=m.join(`
